...
* **L2 loss:** $\Omega(\theta) = \lambda \sum ||W^k||^2_F$ (small weights). **L1 loss:** $\Omega(\theta) = \lambda \sum |W^k|$ (sparse weights).
...
* **LSTM (Long Short-Term Memory):** Solves this with:
    ...
    * **Gates (Sigmoid 0-1):** **Forget ($f_t$)** controls $c_{t-1}$; **Input ($i_t$)** controls new info $\tilde{c}_t$;  
    **Output ($o_t$)** controls $h_t$.
...
* **CBOW:** `Context words -> Target word`. **Skip-Gram:** `Target word -> Context words`.
