...
```{r}
# Example R code to help determine optimal categories using Goodness of Fit
library(classInt)
library(dplyr) # For filter and pull, assuming preprocessed_data is a tibble/dataframe

# Assuming preprocessed_data is already loaded in your R session
# preprocessed_data <- readRDS("preprocessed_pollution_data.rds")

# --- Function to calculate GVF robustly ---
calculate_gvf <- function(data_vector, max_n_classes = 8) {
  # Remove NA values
  ...
  if (length(unique_vals) < 2) {
    message("Not enough unique values for classification. Returning NA for GVF.")
    return(setNames(rep(NA_real_, max_n_classes - 1), paste0("n=", 2:max_n_classes)))
  }

  # Calculate Total Sum of Squares (TSS)
  # Handle case where all remaining values are identical (variance is 0)
  if (var(data_vector) == 0) {
    message("All values are identical, TSS is zero. Returning NA for GVF.")
    return(setNames(rep(NA_real_, max_n_classes - 1), paste0("n=", 2:max_n_classes)))
  }
  tss <- sum((data_vector - mean(data_vector))^2)

  gvf_results <- sapply(2:max_n_classes, function(n) {
    if (length(unique_vals) < n) {
      return(NA_real_) # Cannot create 'n' classes if fewer unique values
    }

    ci <- classIntervals(data_vector, n = n, style = "jenks")

    # Calculate Within-Class Sum of Squares (WCSS)
    # sum(variance_of_class * (size_of_class - 1))
    wcss <- sum(ci$var * (ci$n - 1))
  ...
  names(gvf_results) <- paste0("n=", 2:max_n_classes)
  return(gvf_results)
}

# --- For Air Pollution Deaths ---
air_pollution_data_vector <- preprocessed_data %>%
  pull(death_rate_air_pollution)

print("Goodness of Variance Fit for Air Pollution Deaths (Jenks):")
gvf_air_pollution <- calculate_gvf(air_pollution_data_vector)
print(gvf_air_pollution)

# --- For Mismanaged Plastic Waste Share ---
plastic_waste_data_vector <- preprocessed_data %>%
  pull(mismanaged_plastic_waste_share)

print("\nGoodness of Variance Fit for Mismanaged Plastic Waste Share (Jenks):")
gvf_plastic_waste <- calculate_gvf(plastic_waste_data_vector)
print(gvf_plastic_waste)

# --- How to interpret the results: ---
# Look for values close to 1.0. As you increase 'n' (number of classes),
# GVF should increase or stabilize. The "optimal" number of classes is often
# where the GVF starts to show diminishing returns (an "elbow" in the plot).
# For example, if GVF for n=4 is 0.85 and for n=5 is 0.86, the gain from adding
# an extra class (5 vs 4) might be very small, suggesting 4 classes is sufficient.
# Aim for a balance between high GVF and a manageable number of categories.
```