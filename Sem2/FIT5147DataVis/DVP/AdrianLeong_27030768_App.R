# Student ID: 27030768
# Name: Adrian Leong
# Tutor: Dr. Grace Ting Chai Wen, Applied Session Friday 2-4pm Malaysia

# --- 1. LOAD LIBRARIES ---
# Core libraries for Shiny app functionality, data manipulation, and plotting.
# List of required packages
required_packages <- c(
  "shiny", "dplyr", "ggplot2", "plotly", "leaflet",
    ...
    column(
      ...
      wellPanel(
        ...
        sliderInput("filter_gdp_range", "GDP per Capita Range:",
          ...
          value = c(
            floor(min(preprocessed_data$gdp_per_capita, na.rm = TRUE)),
...
server <- function(input, output, session) {
  ...
  output$dynamic_choropleth_map <- renderLeaflet({
    ...
    limits <- if (selected_var %in% c("mortality_residual_gdp", "plastic_residual_gdp")) {
      if (selected_var == "mortality_residual_gdp") {
    ...
    leaflet(map_data,
      options = leafletOptions(
        maxBounds = list(world_map_bounds$south_west, world_map_bounds$north_east),
        minZoom = 2, maxZoom = 8,
        zoomControl = TRUE
      )
    ) %>%
      ...
      addLegend(
        pal = pal, values = ~ get(selected_var),
  ...
  output$combined_scatter_plot <- renderPlotly({
    ...
    p <- ggplot(
      plot_data,
      aes(
        x = log_gdp_per_capita, y = .data[[y_var_name]],
        color = income_group,
        text = paste0(
          "Country: ", country,
  ...
  output$treemap_plot <- renderPlotly({
    ...
    plot_data <- filtered_data() %>%
      as_tibble() %>%
      filter(
        !is.na(world_gdp_share), world_gdp_share > 0,
  ...
  output$growth_waste_plot <- renderPlotly({
    ...
    p <- ggplot(plot_data, aes(y = country)) +
      geom_point(
        aes(
          x = past_world_gdp_share, color = "Past GDP Share",
      ...
      geom_point(
        aes(
          x = world_gdp_share, color = "Current GDP Share",
      ...
      geom_point(
        aes(
          x = mismanaged_plastic_waste_share, fill = mismanaged_plastic_waste_share,
      ...
      geom_segment(
        aes(
          x = past_world_gdp_share, xend = world_gdp_share,
      ...
      geom_segment(
        aes(
          x = world_gdp_share, xend = mismanaged_plastic_waste_share,
          y = country, yend = country
        ),
        color = "gray", linetype = "dashed"
      ) +
      ...
      labs(
        title = "Economic Share Evolution vs Plastic Waste Contribution (Top 20 Countries)",
        x = "Percentage Value",
        y = "Country",
        color = "Metric"
      ) +
  ...
  observeEvent(event_data("plotly_click", source = "growth_waste_plot"), {
    ...
    if (!is.null(click_data)) {
      ...
      clicked_index <- 21 - click_data$y # click_data$y is ordered bottom up, add 1 because R indexes from 0
  ...
  output$quadrant_plot <- renderPlotly({
    ...
    p <- ggplot(
      plot_data,
      aes(
        x = mismanaged_plastic_waste_share,
        y = death_rate_air_pollution,
        text = tooltip_text,
        key = country
      )
    ) + # 'key' is used for plotly click events.
      ...
      labs(
        title = "Pollution Burden: Who Pollutes vs Who Suffers?",
