...
top_10_category_counts <- top_10_data %>%
...
top_10_category_counts_ordered <- top_10_category_counts %>%
...
vis <- ggplot(top_10_category_counts_ordered, aes(x = Venue.Name, y = Count, fill = Primary.Genre)) +
...
category_colors <- colorFactor(palette = "plasma", domain = top_10_category_counts$Primary.Genre)

# Step 6: Scale the radius based on the count (adjust the multiplier as needed)
scale_factor <- 3 # Adjust this to control the size of the circles


interactive_map <- leaflet() %>%
  setView(lng = 144.962, lat = -37.8162, zoom = 12) %>%
  addTiles() %>%
  addLegend(pal = category_colors, values = top_10_category_counts$Primary.Genre, title = "Primary.Genre") %>%
  addCircleMarkers(data = top_10_category_counts,
  color = ~category_colors(top_10_category_counts$Primary.Genre),
  radius = ~sqrt(top_10_category_counts$Count*scale_factor),
  fillOpacity = 0.2,
  label = ~lapply(paste(
...
# 'Name', 'Category', 'Latitude', 'Longitude', 'StartYear', 'EndYear'
...
top_10_category_counts <- top_10_data %>% group_by(Name, Category, Latitude, Longitude) %>% summarise(Count = n()) %>% ungroup()
category_colors_factor <- factor(unique(top_10_category_counts$Category))
...
m <- leaflet(top_10_category_counts) %>%
  addTiles() %>%
...
stacked_bar_chart <- ggplot(top_10_category_counts_ordered, aes(x = Name, y = Count, fill = Category)) +
  geom_bar(stat = "identity") +
...
ui <- fixedPage(
  ...
  sliderInput(
    ...
    min = min(data$StartYear, data$EndYear, na.rm = TRUE),
    max = max(data$StartYear, data$EndYear, na.rm = TRUE),
    value = c(min(data$StartYear, data$EndYear, na.rm = TRUE), max(data$StartYear, data$EndYear, na.rm = TRUE)),
    step = 1,
    sep = "",
    animate = FALSE
  )
...
server <- function(input, output, session) {
  output$map <- renderLeaflet({
    leaflet(top_10_category_counts) %>%
      addTiles() %>%
      addLegend(pal = category_colors, values = ~Category, title = "Category") %>%
      addCircleMarkers(
        lng = ~Longitude,
  ...
  observe({
    ...
    filtered_map_data <- top_10_data %>%
      filter(StartYear >= min_year & EndYear <= max_year) %>%
